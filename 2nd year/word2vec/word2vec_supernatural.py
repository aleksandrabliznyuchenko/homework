...
def main():
    file = 'supernatural.gexf' ## Название файла, в котррый сохраняем граф и откуда потом будем его подгружать
    words = ['тварь', 'существо', 'создание', 'черт', 'чертик', 'чертенок', 'Дьявол',
             ...
             'феникс', 'оборотень', 'монстр', 'нефилим', 'джинн', 'голем'] ## Список наших слов 
    model = model_creator()
    G =  nodes(model, words)
    edges(model, G, file)
